...
class MenuItem {
public:
	MenuItem(const char* itemName, const char* itemDescription='\0', char shortKey='\0' ) : name(itemName), description(itemDescription), shortkey(shortKey) {
		parent = firstChild = previousSibling = nextSibling = 0;
	}
	...
	inline MenuItem* getPreviousSibling() const { return previousSibling; }
	inline MenuItem* getNextSibling() const { return nextSibling; }


	MenuItem *moveParent() { return parent; }
	MenuItem *moveNextSibling() { return nextSibling; }
	MenuItem *movePreviousSibling() { return previousSibling; }
	MenuItem *moveFirstChild() { return firstChild; }

	MenuItem &addFirstChild(MenuItem &mi) {
		firstChild = &mi;
		mi.parent = this;
		return mi;
	}

	MenuItem &addNextSibling(MenuItem &mi) {
		nextSibling = &mi;
		mi.previousSibling = this;
		mi.parent = parent;
		return mi;
	}

	//default vertical menu
//	MenuItem &add(MenuItem &mi) { if (!firstChild) {return addFirstChild(&mi)} else {return addAfter(mi); }
...
public:
	...
	MenuBackend(cb_use menuUse, cb_change menuChange = 0) : root("MenuRoot","Main Menu") {
...
//	}
	...
	void moveParent() {
		setCurrent(current->moveParent());
	}

	void moveNextSibling() {
		setCurrent(current->moveNextSibling());
	}

	void movePreviousSibling() {
		setCurrent(current->movePreviousSibling());
	}

	void moveFirstChild() {
		setCurrent(current->moveFirstChild());
	}
